...
async def setup(bot: commands.Bot, points_svc: PointsService) -> None:  # type: ignore[override]
    # The points_group is a class-level app_commands.Group, so add_cog registers
    # it automatically. Calling tree.add_command here too would register it twice
    # and raise CommandAlreadyRegistered, so we only call add_cog.
    await bot.add_cog(Points(bot, points_svc))
