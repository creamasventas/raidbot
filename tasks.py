...
async def setup(bot: commands.Bot, tasks_svc: TasksService) -> None:  # type: ignore[override]
    # task_admin is a class-level app_commands.Group, so add_cog registers it
    # automatically. Adding it to the tree here too would double-register and
    # raise CommandAlreadyRegistered, so we only call add_cog.
    await bot.add_cog(Tasks(bot, tasks_svc))
